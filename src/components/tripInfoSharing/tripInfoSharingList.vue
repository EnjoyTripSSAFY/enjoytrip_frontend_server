...
const fetchData = () => {
  ...
  axios
      ...
      .then((response) => {
        tableData.value = response.data.result.boards; // 데이터 구조에 따라서 적절히 수정
        pagination.value.total = response.data.result.totalPageCount;
        param.value.pgno = response.data.result.current;
      })
      .catch((error) => {
